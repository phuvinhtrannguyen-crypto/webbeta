...
import CinematicEvents from '../components/CinematicEvents.jsx';
...
function seatPositions(n) {
  ...
  for (let i = 0; i < n; i += 1) {
    ...
    positions.push({ top: `${cy + ry * Math.sin(t)}%`, left: `${cx + rx * Math.cos(t)}%` });
  }
  return positions;
}
...
export default function Table({ room, setRoom, onLeave, me }) {
  ...
  const [showdownReveals, setShowdownReveals] = useState([]);
  ...
  const orderedPlayers = useMemo(() => {
    ...
    return [...state.players.slice(myIdx), ...state.players.slice(0, myIdx)];
  }, [state, me]);
  const seats = useMemo(() => seatPositions(orderedPlayers.length || 1), [orderedPlayers.length]);

  if (!state) {
    return <div className="table-loading"><p>Đang kết nối phòng…</p></div>;
  }
    ...
    <div className="table-page">
      ...
      <header className="topbar">
        <div className="room-info">
          ...
          <button className="copy-btn" onClick={() => navigator.clipboard?.writeText(state.roomId)} title="Sao chép mã">📋</button>
        </div>
        <div className="hand-info">Ván #{state.handNumber} · {phaseLabel(state.phase)}</div>
        <div className="top-actions">
          <button className={`btn btn-mic ${myPlayer?.micOn ? 'on' : ''}`} onClick={toggleMic} title={myPlayer?.micOn ? 'Tắt mic' : 'Bật mic'}>{myPlayer?.micOn ? '🎙️ Mic' : '🔇 Mic'}</button>
          <button className="btn btn-mixer" onClick={() => { ensureContext(); setMixerOpen(true); }} title="Bộ chỉnh âm">🎚️</button>
          <button className="btn btn-ghost" onClick={onLeave}>Thoát</button>
        </div>
      </header>

      <main className="table-main">
        <div className={`poker-table seats-${orderedPlayers.length >= 13 ? 'xs' : orderedPlayers.length >= 9 ? 'sm' : 'md'}`}>
          <div className="table-felt">
            <div className="pot"><div className="pot-label">POT</div><div className="pot-amount">{state.pot} búng</div></div>
            <div className="community">
              {[0, 1, 2, 3, 4].map((i) => {
                const c = state.community[i];
                const isRiverSlot = i === 4;
                return <div key={i} className={`community-slot ${c ? 'filled' : ''} ${isRiverSlot && c ? 'river-slam' : ''}`} style={{ animationDelay: `${i * 120}ms` }}>{c ? <Card card={c} revealed /> : <Card placeholder />}</div>;
              })}
            </div>
          ...
          {orderedPlayers.map((p, i) => (
            <Seat key={p.id} player={p} position={seats[i] || seats[0]} isMe={p.id === me} isDealer={p.id === state.dealerId} isActing={p.id === state.actingId} actionRemainingMs={p.id === state.actingId ? countdown : null} hole={p.id === me ? hole : null} showdownReveal={revealedIds.has(p.id) ? showdownReveals.find((r) => r.id === p.id) : null} speaking={speakingIds.has(p.id)} />
          ))}
        </div>
        <aside className="side-panel"><ChatPanel messages={chat} onSend={sendChat} meName={myPlayer?.name} /></aside>
      </main>

      <footer className="bottom-bar">
        {errorMsg && <div className="error">{errorMsg}</div>}
        {state.phase === 'waiting' || state.phase === 'finished' ? (
          <div className="waiting-bar"><div>Đang chờ {isHost ? 'bạn bắt đầu ván' : 'chủ phòng bắt đầu ván'}…</div>{isHost && <button className="btn btn-primary" onClick={start}>Bắt đầu ván</button>}</div>
        ) : <BetBar myTurn={myTurn} myPlayer={myPlayer} state={state} onAct={act} countdownMs={countdown} />}
      </footer>
      <AudioMixer players={state.players} me={me} open={mixerOpen} onClose={() => setMixerOpen(false)} />
    </div>
  );
}
